...
if(!class_exists("lmcTextChunk", false))
{
	...
	function lmcArrayCompare($needle, $haystack)
	{
	...
	function lmccallbackSaveArray($array, $encoding)
	{
		...
		$skip |= lmcArrayCompare(array_keys($encoding), $indices);
	...
	class lmcTextChunk
	{
		...
		static private function RegisterStandardChunkTypes()
		{
			...
			lmcTextChunk::RegisterChunk("EHFILE", "TEXT");		// Error Filename - Hashed
            lmcTextChunk::RegisterChunk("EHLINE", "INT");		// Error Filename - Hashed
            ...
            lmcTextChunk::RegisterChunk('VERSION', 'INT');      // Version
            lmcTextChunk::RegisterChunk('VER', 'INT');          // Chunk version
			lmcTextChunk::RegisterChunk("CID", "INT");			// Client ID
            lmcTextChunk::RegisterChunk("SID", "INT");			// Server ID
			lmcTextChunk::RegisterChunk("KID", "INT");			// Key ID
            lmcTextChunk::RegisterChunk("UID", "INT");			// Unique ID
            lmcTextChunk::RegisterChunk("TRNSID", "TEXT");		// Transaction ID
            ...
            lmcTextChunk::RegisterChunk("CCKD", "DDATA");			// Cookie Data
			
			// Token Request Debug
            lmcTextChunk::RegisterChunk("BID", "INT");			// Bucket ID
            lmcTextChunk::RegisterChunk("CID", "INT");			// Captcha ID
            ...
            lmcTextChunk::RegisterChunk("SSERV", "TEXT");		// The Security Server
            lmcTextChunk::RegisterChunk("NUPROXY_URL", "TEXT");		// The NuProxy URL
            ...
            lmcTextChunk::RegisterChunk("LINKS2", "DDATA");		// The links (css, javascript) to output
            ...
            lmcTextChunk::RegisterChunk("ASVALUES2", "DDATA");		// The string array of value to be sent to the NuCaptcha Flash Widget
            ...
            lmcTextChunk::RegisterChunk("THREATLEVEL", "INT");      // Threat level 1000-5000
            lmcTextChunk::RegisterChunk("ESETTINGS", "DDATA");      // Enterprise settings key value pairs
            lmcTextChunk::RegisterChunk("EDATA", "DDATA");          // Enterprise data key value pairs
            
            // Security Response Chunks
            lmcTextChunk::RegisterChunk("AK_RETRY", "INT");         // Authkey retry
            
            // Token Disabled Response Chunks
			lmcTextChunk::RegisterChunk("DISABLED_REASON", "TEXT"); // Is this publisher disabled.  If so, why?

			// Validate Request Chunks
            lmcTextChunk::RegisterChunk( "RESP", "BIN" );           // Response - What the user typed.  Make it binary because we don't trust the user to provide valid text.
			lmcTextChunk::RegisterChunk( "FBACK", "TEXT" );         // User feedback
			lmcTextChunk::RegisterChunk( "DIFFR", "INT" );          // Difficulty rating
			lmcTextChunk::RegisterChunk( "DATA_TYPE", "TEXT" );		// The data format of the capthca (gif, mp3, mp4 etc)
			lmcTextChunk::RegisterChunk( "ANSW_INDEX", "INT" );		// The answer index for this validate
			lmcTextChunk::RegisterChunk( "FENTER", "INT" );         // Timestamp (unix time) the user entered the text input
			lmcTextChunk::RegisterChunk( "FEXIT", "INT" );          // Timestamp (unix time) the user exited the text input
			lmcTextChunk::RegisterChunk( "LANG", "TEXT" );          // The locale for this validate
		...
		static public function Decode(&$tchunk, $expectedtype)
		{
			...
			if( false === $end )
			{
				$msg = 'Invalid lmcTextChunk format ('.$i.'/'.$numchunks.'): ' . var_export($tchunk,true);
				error_log($msg);
				throw new LeapException($msg, LMSC_UNKNOWN);
			}
