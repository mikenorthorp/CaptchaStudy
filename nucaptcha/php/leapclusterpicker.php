...
class lmcClusterPicker
{
    ...
    private static function GetDNS ($clusterRecord)
    {
		...
		// suppress any error output from dns_get_record
		$result = @dns_get_record($clusterRecord, DNS_TXT);
		
		// make sure we get an array back
		if(false === $result || !is_array($result) || 0 == count($result) || !array_key_exists('ttl',$result[0]))
		{
    ...
    {		
		...
		if(false === $result || !is_array($result) || 0 == count($result) || !array_key_exists('txt',$result[0]))
		{
